...
from wwricu.service.manage import confirm_totp, delete_sys_config, enforce_totp, get_sys_config, list_trash, set_sys_config, process_trash, update_admin_user
from wwricu.service.security import require_admin

manage_api = APIRouter(prefix='/manage', tags=['Manage API'], dependencies=[Depends(require_admin)])
...
async def config_set_api(config: ConfigRO):
    if config.value is None:
        await delete_sys_config([config.key])
        return
    await set_sys_config(config.key, config.value)
...
async def config_get_api(key: str) -> str | None:
    ...
    return await get_sys_config(ConfigKeyEnum(key))
...
async def user_config_api(user: UserRO, request: Request):
    ...
    await update_admin_user(user, session_id)
